...
class EvaClassification():
    def __init__(self,class_names:list, none_name:str = 'UN'): #UN is Unknown
        ...
        for idx in union_indices: # since it's union, there is no way to get both unknown.
            if not multi_pred:
                ...
                if len(gt_indices_set)==0 and len(pd_indices_set) == 0: 
                    ...
                    #두 물체 사이의 관계가 존재하지 않다면, Unknown에 해당되는 혼동행렬에 값을 증가시킨다. 즉 26번째의 인덱스 값일거임
                #     (나의 생각)
                # 굳이 None을 예측하지 않아도.. 관계가 없으면 UN에 값이 증가할 듯
                ...
                if len(gt_indices_set) > 0:
                    ...
                    # 정답 관계 범주들 집합과 예측 관계 범주들 집합의 교집합을 통해 Postive한 녀석들만 intersection 집합에 담는다.

                    diff_gt = set(gt_indices_set).difference(intersection) # unmatched gt
                    # 완전히 일치할 경우에 diff_gt는 공집합이 될 것이다.
                    # 정답에는 4번의 관계가 존재한다고 했는데 예측할 땐 실패한 경우 diff_gt에는 {4}라는 값이 남아 있을 거임
                    # 정답은 존재하는데 관계 예측을 못한 경우
                    diff_pd = set(pd_indices_set).difference(intersection) # unmatched pd
                    # 완전히 일치할 경우에 diff_pd는 공집합이 될 것이다.
                    # 예측에는 두 물체 사이에 [4,8,9] 번의 관계들이 존재한다고 했는데 정답에는 없을 때.. diff_pd는 {4,8,9} 값이 남아있을 거임.
                    # 즉 diff_pd --> 예측을 했는데 틀린 경우임
                    # 정답은 존재하지 않는데 예측을 잘못한 경우
                    for i in intersection:
                        self.c_mat[i][i] += 1
                        # positive한 녀석의 인덱스 값에 +1 해줌
                        ...
                        for pd_idx in diff_pd:
                            ...
                            if len(diff_gt) > 0:
                                for gt_idx in diff_gt:
                                    # 하지만 정답이 존재할 경우에 예측이 잘못한 경우에는 +1 보다는 확률 값을 부여함
                                    self.c_mat[gt_idx][pd_idx] += 1/len(diff_gt)
                            else:
                                self.c_mat[self.unknown][pd_idx] += 1
                    #             정답은 존재하지 않는데 예측을 잘못한 경우
                                # 따라서 정답은 unknown, 예측은 한 경우에 +1을 부여함
                    if not pd_only:
                        for gt_idx in diff_gt:
                            if len(diff_pd) > 0:
                                for pd_idx in diff_pd:
                                    self.c_mat[gt_idx][pd_idx] += 1/len(diff_pd)
                            #         정답은 존재하는데 몇 번 관계 범주를 잘못 예측했는지 알 경우
                            else:
                                # 정답은 존재하는데 몇 번 관계 범주를 잘못 예측했는지 모르는 경우
