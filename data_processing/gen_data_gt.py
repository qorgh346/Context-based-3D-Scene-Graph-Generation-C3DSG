...
def Parser(add_help=True):
    parser = argparse.ArgumentParser(description='Process some integers.', formatter_class = argparse.ArgumentDefaultsHelpFormatter,
                                     add_help=add_help)
    parser.add_argument('--scans', type=str,default='/home/baebro/3RScan/data/new_3RScan')
    parser.add_argument('--type', type=str, default='test', choices=['train', 'test', 'validation'], help="allow multiple rel pred outputs per pair",required=False)
    parser.add_argument('--pth_out', type=str,default='../data/3RScan160', help='pth to output directory',required=False)
    ...
    parser.add_argument('--min_segs', type=int,default=256,help='Minimum segments for each segGroup')
...
def gen_relationship(scan_id:str,split:int, map_segment_pd_2_gt:dict,instance2labelName:dict,
    ...
    if gt_relationships is not None:
        ...
        for rel in gt_relationships:
            ...
            split_relationships.append([ int(id_src), int(id_tar), idx_in_txt, name ])
