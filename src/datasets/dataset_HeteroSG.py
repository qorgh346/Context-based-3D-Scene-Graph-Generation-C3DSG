...
from utils import util_ply, util_data, util, define,hetero_util_data
...
def dataset_loading_3RScan(root: str, pth_selection: str, split: str, class_choice: list = None):
    ...
    #add 23.02.01
    with open(os.path.join(root, 'predefined_domain_knowledge.json'), 'r') as f:
        json_data = json.dumps(json.load(f))
        gt_knowledge_data = json.loads(json_data)

    data = dict()
    data['scans'] = data1['scans'] + data2['scans']
    data['neighbors'] = {**data1['neighbors'], **data2['neighbors']}
    return classNames, relationNames, data, selected_scans,gt_knowledge_data
...
class RIODatasetHeteroGraph(data.Dataset):
    def __init__(self,
                 config,
                 split='train_scans',
        ...
        with open(os.path.join(self.root[0], 'room_type.json'), 'r') as f:
            data = json.load(f)
            self.scene_type = data['scene_types']
        ...
        if isinstance(self.root, list):
            ...
            for i in range(len(self.root)):
                ...
                l_classNames, l_relationNames, l_data, l_selected_scans, l_pre_knowledge = \
            ...
            classNames, relationNames, data, selected_scans,pre_knowledge = \
        ...
        self.prior_knowledge = l_pre_knowledge
        ...
        if self.config.WORD_EMBEDDING:
            # glove_file_name = 'glove.6B.5d.txt.word2vec'
            self.embedding_model = hetero_util_data.build_word_embedding(self.config.WORD_VECTOR_TEXT)
    ...
    def __getitem__(self, index):
        ...
        scene_type = self.scene_type[scan_id_no_split]
        obj_points, descriptor, edge_indices, instance2mask, gt_rels, gt_class, heteroGraphdata = \
            hetero_util_data.data_preparation(points, instances, selected_instances,
                                       ...
                                       use_all=self.for_eval, glove_model=self.embedding_model,
                                       use_predict_class=self.config.MODEL.USE_class_categories, scene_type=scene_type,
                                       prior_knowledge = self.prior_knowledge)
        # print('gt_rel_size = ',gt_rels.size())
        # for i in range(gt_rels.size(0)):
        #     print(' {} = {}'.format(i,torch.sum(gt_rels[i,:])))
        # scan_id, instance2mask, obj_points, descriptor, gt_class, gt_rels, edge_indices
        heteroGraphdata.scan_id = scan_id
...
if __name__ == '__main__':
    ...
    config.dataset.root = '../data/hobeNew/'  # gen_data'
    ...
    use_rgb = False
    use_normal = True
    dataset = RIODatasetGraph(config, split='train_scans', multi_rel_outputs=True, load_cache=config.dataset.load_cache,
