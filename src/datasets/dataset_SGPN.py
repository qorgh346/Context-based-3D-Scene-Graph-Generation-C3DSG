...
from utils import util_ply, util_data, util, define,pointcloud_transform
...
class RIODatasetGraph(data.Dataset):
    def __init__(self,
        ...
        self.pt_v2_transform = pointcloud_transform.Compose(define.test_transform)
        ...
        if self.config.WORD_EMBEDDING:
            ...
            self.embedding_model = util_data.build_word_embedding(self.config.WORD_VECTOR_TEXT)
    ...
    def __getitem__(self, index):
        ...
        obj_points, descriptor, edge_indices, instance2mask, gt_rels, gt_class, obj_pair_indices = \
            util_data.data_preparation(points, instances, selected_instances,
                                       ...
                                       use_all=self.for_eval, union_bbox_points = self.config.union_bbox_point_feature,
                                       use_predict_class=self.config.MODEL.USE_class_categories, scene_type=scene_type,
                                       )
        if self.mconfig.use_total_point:
            data_dict = {'coord':points[:,:3],
                         'color':points[:,3:]}
            data_dict = self.pt_v2_transform(data_dict)

            data_dict['instances'] = instances.astype(np.int)
            data_dict['instance2mask'] = instance2mask
            data_dict['obj_pair_indices'] = obj_pair_indices

        return scan_id, instance2mask, obj_points, descriptor, gt_class, gt_rels, edge_indices,data_dict
...
if __name__ == '__main__':
    ...
    config.dataset.root = '../data/3RS160/'  # gen_data'
