...
from cuda_lib.lib.pointops.functions import  pointops
# from lib.pointops.functions import pointops
from utils.Paconv_scene_seg_util import block
# from utils.Paconv_scene_seg_util
import paconv
...
if __name__ == "__main__":
    ...
    xyz = torch.randn(512, 9, 3, requires_grad=True).cuda()
    xyz_feats = torch.randn(512, 9, 6, requires_grad=True).cuda()

    test_module = PointNet2SAModuleMSG(npoint=4096, radii=[5.0, 10.0], nsamples=[6, 3], mlps=[[9, 3], [9, 6]],args=args)
    test_module.cuda()
    print(test_module(xyz, xyz_feats)[0].size())

    print(test_module(xyz, xyz_feats)[1].size())
