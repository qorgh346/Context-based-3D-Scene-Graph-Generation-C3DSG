...
class MultiHeadedEdgeAttention(torch.nn.Module):
    ...
    def forward(self, query, edge, value):
        # print('query = ',query.size()) #Gen_Index에서 edgeindex 만큼 message Passing 후 사이즈 : (edgeIndex num , diM)
        # print('edge = ', edge.size())
        # print('value = ', value.size())
        # sys.exit()
        batch_dim = query.size(0)
        #query = subject feature
        #edge = predicate feature
        #value = object feature
        t = torch.cat([query,edge,value],dim=1)
        # print(t.size())
        # print(self.nn_edge)
        # sys.exit()
        #
        ...
        if self.attention == 'fat':
            value = self.proj_value(value) #object feature
            query = self.proj_query(query).view(batch_dim, self.d_n, self.num_heads) #query =subject feature
            edge = self.proj_edge(edge).view(batch_dim, self.d_e, self.num_heads)

            if self.use_edge:
                prob = self.nn(torch.cat([query,edge],dim=1)) # b, dim, head  --> Attention Score
...
class GraphEdgeAttenNetwork(BaseNetwork):
    ...
    def forward(self, x, edge_feature, edge_index):
        ...
        #self.index_get : Edge_index 상에서의 source -- target을 나눠서 x_i, x_j 로 return
        x_i, x_j = self.index_get(x, edge_index)
        # print(x_i.size())
        # print(x_j.size())

        # print(edge_feature.size())
        # print(self.edgeatten)
        # sys.exit()
...
class GraphEdgeAttenNetworkLayers(torch.nn.Module):
    ...
    def __init__(self, dim_node, dim_edge, dim_atten, num_layers, num_heads=1, aggr= 'max', 
        ...
        super().__init__()
        self.num_layers = num_layers # num_layer = 2
        self.num_heads = num_heads #num_head = 4
