...
from CSGGN.src.datasets.DataLoader import CustomDataLoader
# from model_SGFN import SGFNModel
# from model_SGPN import SGPNModel
from CSGGN.src.models.model_CSGGN import CSGGNModel
from CSGGN.src.datasets.dataset_builder import build_dataset
from torch.utils.tensorboard import SummaryWriter
from config import Config
import op_utils
from CSGGN.utils import plot_confusion_matrix
from CSGGN.utils import util_eva
...
class CSGGN():
    def __init__(self, config,args):
        ...
        if self.use_edge_descriptor: #True
            ...
            self.model = CSGGNModel(config,self.model_name,num_obj_class,num_rel_class,self.className,self.obj_w,self.rel_w).to(config.DEVICE)
        else:
            self.model = CSGGNModel(config,self.model_name,num_obj_class,num_rel_class,self.className).to(config.DEVICE)
    ...
    def train(self):
        ...
        while(keep_training):
            ...
            for items in loader:
                ...
                if self.use_edge_descriptor:
                    obj_points,descriptor,gt_obj_cls,gt_rel_cls,edge_indices = self.data_processing(items[2:-1])
                    # print(obj_points.size()[0])
                    for name, value in items[-1].items():
                        if name in ['instance2mask', 'obj_pair_indices']: continue
                        items[-1][name] = value.to(self.config.DEVICE)

                    total_point_num = items[-1]['coord']
                    print('total---',total_point_num.size(1))
                    if total_point_num.size(1) >= 150000:
                        print('total_point_num = {} pass'.format(total_point_num.size(1)))
                        continue
                ...
                if self.use_edge_descriptor:
                    # sys.exit()
                    if self.config.dataset.use_total_point:
                        logs, pred_obj_cls, pred_rel_cls, probs = self.model.process(obj_points, descriptor,edge_indices.t().contiguous(),
                        ...
                        logs, pred_obj_cls, pred_rel_cls, probs = self.model.process(obj_points, descriptor,
                ...
                if self.config.LOG_IMG_INTERVAL and iteration % self.config.LOG_IMG_INTERVAL == 0:
                    img_confusion_matrix = plot_confusion_matrix.plot_confusion_matrix(eva_tool.eva_o_cls.c_mat,
                                                                                       ...
                                                                                       plot_text=False,
                                                                                       plot=False)
                    self.writter.add_figure('train_obj_confusion_matrix', img_confusion_matrix,
                    ...
                    img_confusion_matrix = plot_confusion_matrix.plot_confusion_matrix(eva_tool.eva_r_cls.c_mat,
                                                                                       ...
                                                                                       plot_text=False,
                                                                                       plot=False)
                    self.writter.add_figure('train_rel_confusion_matrix', img_confusion_matrix,
                                            global_step=self.model.iteration)
                    ...
                    for i in range(self.model.mconfig.N_LAYERS):
                        ...
                        img_score_matrix = plot_confusion_matrix.plot_confusion_matrix(
                            ...
                            plot=False,
                            normalize=False)
                        self.writter.add_figure('train_probs_' + str(i), img_score_matrix,
                                                global_step=self.model.iteration)
    ...
    def validation(self, debug_mode = False):
        ...
        for i, items in enumerate(val_loader, 0):
            ...
            if self.use_edge_descriptor:
                ...
                total_point_num = items[-1]['coord']
                # print('total---', total_point_num.size(1))
                if total_point_num.size(1) >= 180000:
                    print('total_point_num = {} pass'.format(total_point_num.size(1)))
                    continue
            ...
            with torch.no_grad():
                if self.use_edge_descriptor:
                    result = self.model(obj_points, descriptor,
                                        edge_indices.t().contiguous(),data_dict=items[-1],
                    ...
                    pred_obj_cls, pred_rel_cls, obj_feature, rel_feature, gcn_obj_feature, gcn_rel_feature, probs = \
                    result['obj_pred'],result['rel_pred'],result['obj_feature'],result['rel_feature'],result['gcn_obj_feature'],result['gcn_rel_feature'],result['probs']
        ...
        for i, items in enumerate(val_loader, 0):
            ...
            with torch.no_grad():
                if self.use_edge_descriptor:
                    ...
                    result = self.model(obj_points, descriptor,edge_indices.t().contiguous(),data_dict=items[-1],return_meta_data= True)
                    pred_obj_cls, pred_rel_cls, obj_feature, rel_feature, gcn_obj_feature, gcn_rel_feature, probs = \
                        result['obj_pred'], result['rel_pred'], result['obj_feature'], result['rel_feature'], result[
                            'gcn_obj_feature'], result['gcn_rel_feature'], result['probs']
